...
// Except JV_CRYPTO_cbc128_decrypt, these codes are from KISA.
static uint32_t SS0[256] = {
...
// Code from OpenSSL 1.0.2
void JV_CRYPTO_cbc128_decrypt(uint8_t *in, uint8_t *out, size_t len, const void *key, uint8_t ivec[SeedBlockSize], block128_f block)
{
    size_t n;
	unsigned char tmp[16];

// Input, Output, Key, Vector중 NULL이 있는지 검사한다.
...
// 한번에 16바이트씩 처리
        len -= 16;
// 포인터 주소를 16씩 바꾼다. 커서를 16바이트씩 이동하는 격이다.
        in += 16;
        out += 16;
    }
}
...
/*
void JV_SEED_CBC128_Decrypt_SingleThread(uint8_t *in, uint8_t *out, size_t len, const void *key, uint8_t ivec[SeedBlockSize])
{
	size_t n;
	unsigned char tmp[16];
...
// Do SEED decipher!
		uint32_t L0, L1, R0, R1;		// Iuput/output values at each rounds
		uint32_t T0, T1;				// Temporary variables for round function F
		const uint32_t *K = key;				// Pointer of round keys
...
// Code Fusioned - OpenSSL (CBC128) + KISA (SEED Decrypt) / Cal only one block
// == CONVERT TO OPENCL ==
void JV_SEED_CBC128_Decrypt_OneBlock(uint8_t *in, uint8_t *out, const uint32_t *K, uint8_t ivec[SeedBlockSize])
{
	size_t n;
	uint8_t tmp[16];
	uint32_t* midtmp = NULL;
	uint32_t L0, L1, R0, R1;		// Iuput/output values at each rounds
	uint32_t T0, T1;				// Temporary variables for round function F

// Input, Output, Key, Vector중 NULL이 있는지 검사한다.
    assert(in && out && K && ivec);
